...
class TimeData:
    ...
    def time_schedule(self):
        created_stamp = self.x.published_at.replace("Z", "+00:00")
        created = datetime.fromisoformat(created_stamp).astimezone(jst)
        created_str = created.strftime("%Y/%m/%d %H:%M:%S")
        scheduled_stamp = self.x.start_scheduled.replace("Z", "+00:00")
        scheduled = datetime.fromisoformat(scheduled_stamp).astimezone(jst)
        scheduled_timestamp = discord.utils.format_dt(scheduled, style="f")
        scheduled_date = datetime.strftime(scheduled, "%Y年%m月%d日")
        scheduled_time = datetime.strftime(scheduled, "%H時%M分")
